...
class QRCardGenerator:
    ...
    def generateQrCard(customerObject):
        try:
            ...
            qrCode.svg("backup/qr/{}.svg".format(customerObject.customerID), scale=8, background='#fff')
            #Convert SVG to PNG
            cairosvg.svg2png(url="backup/qr/{}.svg".format(customerObject.customerID), write_to="backup/qr/{}.png".format(customerObject.customerID))
            qrCode = Image.open("backup/qr/{}.png".format(customerObject.customerID))
            qrCode = qrCode.resize((177,177))
            qrCard = Image.open('assets/qr_template.png')
            #Paste QR Code in Card 
            qrCard.paste(qrCode,(401,171)) #Co-Ordinates are calculated from Card Image
            ...
            #Write Name in Card
            writeCard.text((28,321), customerObject.name, font=nameFont, fill=(255,255,255))
            #Write Account in Card
            writeCard.text((28,242), customerObject.accountNo, font=numberFont, fill=(255,255,255))
            qrCard.save("backup/card/{}.png".format(customerObject.customerID))
            return True
